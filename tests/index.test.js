...
test.only('Users who are logged IN are redirected to the petition page when they attempt to go to the registration', () => {
    ...
    return supertest(index.app).get(ROUTES.REGISTER).expect(ROUTES.PETITION)
})

test.only('Users who are logged in are redirected to the petition page when they attempt to go to the Login', () => {
    cookieSession.mockSessionOnce({
        'userId': 'funky'
    })
    /* req.session.chicken will be 'funky' in the following request */
    return supertest(index).get(ROUTES.LOGIN).expect('location', ROUTES.PETITION)
})

// A super test
test('GET / Login returns true', () => {
    // Supertest acts as the client
    return supertest(index)
        .get(ROUTES.LOGIN)
        .then((result) => {
